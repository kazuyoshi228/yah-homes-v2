...
> v0.4（2026-09-06）／status: **稼働中**
> v0.4 変更点（実装済み・デプロイ済み）:
> - シート記入を全列自動化: B/C・E/F（差引後の組数泊数）／**H=直販販売数（当日行）**／I/K/M=先付け泊数／**J/L/N=先付け率**（数式のドラッグ事故を翌朝自動修復するため）
> - **状態保存を差分計算の直後へ移動**（旧: シート記入・メールの後）。途中失敗時の二重計上を根絶
> - request/inquiry→confirmed の遷移を新規として計上（旧: どの分岐にも該当せず取りこぼし）
> - **物理削除の検知**（前回スナップショットから消えた予約）。誤検知対策として①取得窓の後端から外れた過去予約は除外 ②ID指定で再照会し実在しない場合のみ削除と判定
> - 除外語に 山田/杉本/sugimoto を追加し、判定対象を氏名＋referer＋apiSource に拡大。**API直作成は原則除外**するが、直販（custom1が"yah.homes direct"）は正規ゲストとして採用し明細に [直販] 表示
> - 週次: 本番2物件のみに限定／週の起点をJST化／キャンセルは cancelTime 基準／手渡し比率の分子をAirbnb経由に限定／**3ヶ月稼働率カード（当月・次月・次々月）を追加**／**直販の件数・売上をGA4ではなくFirestore台帳から集計**（社内テスト混入の排除・[analysis_direct_funnel_202609.md](analysis_direct_funnel_202609.md) §3）
> - 日次メールの先頭カードを差引後→**グロス新規（件数＋泊数＋明細を最上段）**に変更（大型新規が取消と相殺されて見えない問題）
