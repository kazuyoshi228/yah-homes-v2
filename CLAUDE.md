...
| 本番 | `main` | `firebase deploy --only hosting` | https://yah.homes （現状は `https://yah-homes.web.app`。独自ドメイン未接続） |
...
- 未了：`firebase init`（`yah-homes`）・`firebase.json`（redirects で `/home` 系→301・trailingSlash 整合）・独自ドメイン接続・dev チャンネル初回デプロイ（URL 未確定）。`firebase login --reauth` 待ち。
